...
#align(center)[
  $ U_(upright("ind")) = -(d Phi)/(d t) = (Delta Phi)/(Delta t) $
]
\
Die induzierte Spannung hängt von der zeitlichen änderung des magnetischen Flusses ab.\

Wenn der Stromkreis N dicht beieinander liegende Schleifen enthaelt, gilt:
#align(center)[
  $ U_(upright("ind")) = -N dot (d Phi)/(d t) ~= -N dot (Delta Phi)/(Delta t) $
]
Grundformel:
#align(center)[
  $ U_(upright("ind")) = -N dot (d)/(d t) (B dot A dot cos(theta)) $
]
\

Eine Spannung wird nur induziert, wenn sich der Fluss aendert. Fuer homogenes Feld gilt:
#align(center)[
  $ Phi = B dot A dot cos(theta) $
]
\
\
